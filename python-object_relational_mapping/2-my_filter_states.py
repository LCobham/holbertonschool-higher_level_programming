...
"""
    ...
    match a name passed as a command line arg
"""
import MySQLdb
import sys
...
if __name__ == "__main__":
    if len(sys.argv) < 5:
        ...
        print("USAGE: ./2-filter_states.py username password database name")
        sys.exit(1)
    ...
    with MySQLdb.connect("localhost", usr, pswd, database, 3306) as con:
        cur = con.cursor()
        cur.execute(
            "SELECT * FROM states WHERE BINARY name = \'{}\'".format(name))
        res = cur.fetchall()
        for row in res:
            print(row)
